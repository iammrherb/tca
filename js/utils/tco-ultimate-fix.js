/**
 * Total Cost Analyzer Ultimate Fix
...
(function() {
  ...
  if (window._tcoFixApplied) {
    console.log('TCO Fix already applied, skipping duplicate initialization');
    return;
  }
  ...
  console.log('ULTIMATE FIX: Starting Total Cost Analyzer fixes...');
  
  // Kill any existing chart-related timeouts to prevent conflicts
  if (window.chartFixTimeouts) {
    Object.keys(window.chartFixTimeouts).forEach(function(key) {
      clearTimeout(window.chartFixTimeouts[key]);
    });
  }
  
  // Prevent other scripts from initializing charts
  if (window.safeInitCharts) {
    window._originalSafeInitCharts = window.safeInitCharts;
    window.safeInitCharts = function() {
      console.log('Blocked competing chart initialization from safeInitCharts');
    };
  }
  
  // Create a debug logger
  const logDebug = function(message) {
    console.log('%c[ULTIMATE FIX] ' + message, 'color: #8e44ad; font-weight: bold;');
  };
  ...
  function initializeCharts() {
    ...
    Object.keys(chartConfigs).forEach(function(chartId) {
      ...
      try {
        ...
        if (config.type === 'radar' && parseInt(Chart.version) < 3) {
    ...
    if (typeof window.EnhancedCalculator !== 'function') {
      ...
      window.EnhancedCalculator = function() {
        ...
        this.calculateAllVendors = function(params) {
          ...
          const current = results[currentVendor];
  ...
  function showSuccessNotification() {
    ...
    notification.innerHTML = 'Total Cost Analyzer repaired successfully!';
  ...
  window.addEventListener('load', function() {
    setTimeout(function() {
      ...
      const recheck = false;
      Object.keys(getChartConfigs()).forEach(function(chartId) {
        const container = document.getElementById(chartId);
        if (container && (!container._chart || !container.querySelector('canvas'))) {
          recheck = true;
        }
      });
